...
func ArrayRange(ctx *object.EvalContext, args ...object.Object) object.Object {
	if len(args) != 2 && len(args) != 1 {
		return &object.Error{Line: ctx.Line, Column: ctx.Column, Message: "range() takes 1 or 2 arguments"}
	}
	...
	if len(args) == 1 {
		...
		if !ok {
			return &object.Error{Line: ctx.Line, Column: ctx.Column, Message: "range() requires an integer"}
		}
		start = 0
		stop = stopArg.Value
	} else {
		...
		if !ok {
			return &object.Error{Line: ctx.Line, Column: ctx.Column, Message: "range() requires integer arguments"}
		}
